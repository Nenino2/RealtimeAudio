...
export function Mediabuffer(element, progressCallback, readyCallback, disableMobileCheck, forceFullDownload, catchup) {
...
Mediabuffer.prototype.getAverageDelay = function () {
	if (this.delays.length) {
		let sum = this.delays.reduce(function (a, b) { return a + b; });
		let avg = sum / this.delays.length;
